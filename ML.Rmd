...
principalcomponents = py_norm_nohrv[["Mole_Expired_sum", "headrot_max", "max_diameter", "SpawnedMole_mean", "Misses_mean", "Hits_sum", "file_path", "condition", "Timestamp"]]
principalcomponents.name = "principalcomponents"
for i in namearray:
  regress(principalcomponents, py_normalised_phys, principalcomponents.name, i)
  print(i)
...
```{python regression}
x = py_norm_nohrv.drop(["file_path", "condition", "Timestamp"], axis=1)
y = py_normalised_phys["EDA"]

X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=.2, random_state=42)


tes = ExtraTreesRegressor(n_estimators=100, random_state=0).fit(X_train, y_train)
tes.score(X_test, y_test)


helpme = pd.DataFrame(x.columns)
helpme["importance"] = tes.feature_importances_
helpme


for s in range(1, 22):
  isolated_NOHRV = py_norm_nohrv[py_norm_nohrv["file_path"] == s]
  isolated_NOHRV = isolated_NOHRV.drop(["file_path", "condition", "Timestamp"], axis=1)
  isolated_phys = py_normalised_phys[py_normalised_phys["file_path"] == s]
  isolated_phys = isolated_phys["EDA"]
  tes.score(isolated_NOHRV, isolated_phys)
