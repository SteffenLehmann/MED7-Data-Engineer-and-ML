...
#py_windowed_physio = py_physio_Data.groupby("file_path")
#py_physio_Data = py_physio_Data.assign(cvnni = 0)
#py_physio_Data = py_physio_Data.assign(cvsd = 0)
...
#py_physio_Data = py_physio_Data.assign(nni_20 = 0)
#py_physio_Data = py_physio_Data.assign(nni_50 = 0)
#py_physio_Data = py_physio_Data.assign(pnni_20 = 0)
#py_physio_Data = py_physio_Data.assign(range_nni = 0)
#py_physio_Data = py_physio_Data.assign(rmssd = 0)
#py_physio_Data = py_physio_Data.assign(sdnn = 0)
#py_physio_Data = py_physio_Data.assign(sdsd = 0)
#py_physio_Data = py_physio_Data.assign(std_hr = 0)
#py_physio_Data = py_physio_Data.assign(mean_hr = 0)
#py_physio_Data = py_physio_Data.assign(pnni_50 = 0)

py_physio_Data["file_path"] = pd.to_numeric(py_physio_Data["file_path"])
py_physio_Data = py_physio_Data[py_physio_Data.file_path == 1]
...
pd.rolling_apply(py_physio_Data["IBI"], "30s", hrv.get_time_domain_features)

test = pd.DataFrame(py_physio_Data.apply(func = hrv.get_time_domain(py_physio_Data["IBI"])).rolling("30s"))
...
r_sample_Window <- r_sample_Window %>% dplyr::select(Timestamp, file_path, headdist, eyedist, righthanddist, lefthanddist)
...
#r_physio_Window$Timestamp = ave(r_physio_Window$headdist, r_physio_Window$file_path, FUN = seq_along)

#beat_vectors<- r_physio_Window %>% split(.$file_path) %>% map(pull, Timestamp)

#test23 = as.numeric(difftime(r_physio_Window$Timestamp, IBI_DF$r_physio_Window[1] , units ="secs"))
...
IBI_DF <- filter(IBI_DF, file_path == "20")
...
hrv.data = AddEpisodes(hrv.data, InitTimes = c(0,300,500), Tags = c("A","B","C"), Durations = c(300,200,130), Values = c(0,0,0))
...
hrv.data = CalculatePowerBand(hrv.data, indexFreqAnalysis= 1, size = 30, sizesp = 45, shift = 1, type = "fourier", ULFmin = 0, ULFmax = 0.03, VLFmin = 0.03, VLFmax = 0.05, LFmin = 0.05, LFmax = 0.15, HFmin = 0.15, HFmax = 0.4 )
#Time domain
hrv.data$TimeAnalysis[[1]]$SDNN #SDNN
hrv.data$TimeAnalysis[[1]]$rMSSD #rMSSD
#Frequency domain
LF <- hrv.data$FreqAnalysis[[1]]$LF # LF
HF <- hrv.data$FreqAnalysis[[1]]$HF #HF
LFHF <- hrv.data$FreqAnalysis[[1]]$LFHF #LFHF

PlotNIHR(hrv.data, Tag="all")
test = PlotPowerBand(hrv.data, indexFreqAnalysis = 1, ymax = 5000, ymaxratio = 50,Tag = "all")
