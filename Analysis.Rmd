...
df <- list.files(path = "./physlogs", full.names = T) %>%
map_dfr(read_csv, .id = "file_path") %>%
group_by(file_path) %>%
dplyr::mutate(Participant = group_indices())
...
dfSummary <- df %>% dplyr::group_by(Participant) %>%
   select(Participant, EDA, IBI) 

dsSummary <- ds %>% dplyr::group_by(Participant) %>% select(-Framecount, -SessionID, -Email, -ParticipantId, -TestId, -PlayedPattern)
