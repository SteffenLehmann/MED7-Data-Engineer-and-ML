...
#DF for HRV
IBI_DF <- physiological_Summary %>% dplyr::select(IBI)#, -Timestamp
IBI_DF <- filter(IBI_DF, IBI > -1 & IBI <= 1500)

#IBI_DF_1 <- filter(IBI_DF, file_path == "1")
#IBI_DF_1 <- IBI_DF_1 %>% na.omit()

IBI_Vector <- IBI_DF$IBI

LoadBeatVector <- function (HRVData, beats, scale = 1,
                             datetime = "1/1/1900 0:0:0"){
  if (HRVData$Verbose) {
    cat("** Loading beats positions **\n")
  }
  dir = getwd()
  on.exit(setwd(dir))
  if (HRVData$Verbose) {
  ...
  if (length(beatsaux) != length(beats)) {
    if (HRVData$Verbose) {
      cat("   Removed", length(beatsaux) - length(beats), 
          "duplicated beats\n")
    }
  }
