...
py_test1 = r.test1
test1 = pd.DataFrame(py_test1)
window = "30s"
resulttest = pd.DataFrame
resulttest2 = pd.DataFrame

test1.index = pd.to_datetime(test1["Timestamp"])
test1 = test1.resample("1s").sum().fillna(0)
#test1["Timestamp"] = pd.to_timestamp(test1["Timestamp"])
#test1.index = test1['Timestamp'].apply(lambda x: pd.Timestamp(x))
#test1["Timestamp"] = test1["Timestamp"].to_timestamp
resulttest = test1.rolling(window).sum()
resulttest2 = test1.resample(window).sum()

resulttest = test1.rolling(window).sum().resample(window).first
type(window)
#test1 = test1.rolling(10).sum()

```

```{python this is test area pls ignore, include=FALSE}
ts = pd.Timestamp('2017-01-01 09:10:11')
ts + pd.DateOffset(seconds=30)

py_test1 = r.test1
test1 = pd.DataFrame(py_test1)

test1 = test1.rolling(pd.DateOffset(seconds=30)).sum()
```
