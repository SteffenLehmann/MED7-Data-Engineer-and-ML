...
```{r Eye trackubg loss analysis, echo=FALSE,message=F, warning=F,results='hide', include=F}
...
#Old version
IBI_DF <- filter(physio_DF, file_path == "1")
...
hrv.data = AddEpisodes(hrv.data, InitTimes = c(13,300,500), Tags = c("A","B","C"), Durations = c(300,200,130), Values = c(0,0,0))
...
#TODO create new episodic vectors for the time domain
...
#Frequency domain. the vector lenght is seconds and the missing rows(26) are the artefacts from the windowing, so the vector is missing 13 rows in each end
