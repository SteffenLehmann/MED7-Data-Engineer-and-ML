...
pupil_data[pupil_data == NA] <- "0.0000"
pupil_data <- sapply(pupil_data, as.double)
pupil_data <- pupil_data %>% dplyr::mutate(TrackLoss1 = dplyr::case_when(diameter == 0 ~ TRUE, TRUE ~ FALSE))
pupil_data <- pupil_data %>% dplyr::mutate(TrackLoss2 = dplyr::case_when(diameter_3d == 0 ~ TRUE, TRUE ~ FALSE))

test <- pupil_data %>% dplyr::group_by(TrackLoss1, file_path) %>% dplyr::summarise(Number=(n())) 
test2 <- count(pupil_data, "file_path")
test <- test %>% filter(TrackLoss1 == TRUE)
test$percentage <- (test$Number / test2$freq) * 100

```

```{r pupil data pre-process, echo=FALSE,message=F, warning=F,results='hide', include=F}
pupil_summary$pupil_timestamp <- round(pupil_summary$pupil_timestamp)
pupil_summary <- pupil_summary %>% filter(diameter < 101)
pupiltest <- pupil_summary %>% group_by(file_path, pupil_timestamp) %>% mutate_each(funs(mean)) %>% distinct
...
py_windowed_events = py_event_data.groupby("file_path").resample("1s").mean()
...
py_windowed_samples = py_sample_data.groupby("file_path").resample("1s").mean()
...
py_sliding_samples = py_windowed_samples
...
py_physio_Data = py_physio_Data.groupby("file_path").resample("1s").mean()
...
pupiltest$Timestamp = ave(pupiltest$diameter, pupiltest$file_path, FUN = seq_along)
pupiltest %>% 
  ggplot( aes(x=Timestamp, y=diameter, group=factor(file_path), color=factor(file_path))) +
    #geom_line() +
    #scale_fill_discrete(labels = c(c(unique(r_sample_Window$file_path)))) + 
    ggtitle("pupil size") +
    theme_ipsum() +
...
gamedf <- gamedf %>% dplyr::select(-diameter_3d, -pupil_timestamp, -lefthanddist)
